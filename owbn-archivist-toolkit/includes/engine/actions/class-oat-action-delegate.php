...
class OAT_Action_Delegate {
    ...
    public static function execute( $entry, $user_id, $data = array() ) {
        ...
        // Mark current user's assignment as delegated.
        $assignees = OAT_Assignee::for_entry_step( (int) $entry->id, $step );
        foreach ( $assignees as $a ) {
            if ( (int) $a->user_id === $user_id && $a->status === 'pending' ) {
                OAT_Assignee::update_status( (int) $a->id, 'delegated' );
                break;
            }
        }

        // Assign the delegate (can be outside normal pool per D-027).
        ...
        OAT_Timeline::append( array(
            ...
            'action_type'     => OAT_Constants::ACTION_DELEGATE,
            'actor_id'        => $user_id,
            'step'            => $step,
